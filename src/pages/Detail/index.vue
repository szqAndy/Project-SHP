        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前我们的路由跳转，从A路由跳转到B路由，这里在加入购物车，进行路由跳转之前，发请求把你购买的产品的信息请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      /*
        当前这里是派发一个action，也想服务器发请求，判断加入购物车是成功还是失败，进行相应的操作，
        his.$store.dispatch("addOrUpdateShopCart", {skuId: this.$route.params.skuid, idskuNum: this.skuNum});
        上面这行代码说白了：调用仓库中的addOrUpdateShopCart这个方法加上async，返回一定是一个promise
        此刻：要么成功|要么失败
      */
      // 2：服务器存储成功--进行路由跳转传递参数，失败，给用户进行提示
      try {
        // 成功
        await this.$store.dispatch("addOrUpdateShopCart", {
          skuId: this.$route.params.skuid, // 商品id
          skuNum: this.skuNum, // 件数
        });
        // 3：进行路由跳转
        // 4：在路由跳转时还需要将产品的信息给下一级的路由组件
        /*
          一些简单的数据skuNum，通过query形式给路由组件传递过去
          产品信息的数据【比较复杂：skuInfo】，通过会话存储（会话结束数据会消失）
          // 本地存储|会话存储，一般存储的是字符串
        */

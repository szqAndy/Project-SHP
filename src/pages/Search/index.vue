    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
          ...
          <!-- 分页器 当前页：pageNo；连续页：continues；总共多少条数据：total； 显示多少数据：pageSize-->
          <pagination
...
export default {
  ...
  data() {
    return {
      ...
      searchParams: {
        ...
        categoryName: "", // 分类名字
        keyword: "", // 关键字
        order: "2:asc", // 排序：初始状态应该是综合|降序
        pageNo: 1, // 分页器
        pageSize: 10, // 代表的是每一页展示数据的个数
        props: [], // 平台售卖属性操作携带的参数
        trademark: "", // 品牌
      },
    };
  ...
  mounted() {
    // 组件挂载完毕执行一次【仅仅执行一次】
    // 在发请求之前给服务器参数【searchParams参数发生变化有数据带给服务器】
    this.getData();
  },
  computed: {
  ...
  methods: {
    // 向服务器发请求获取search模块数据（根据参数不同返回的数据进行展示）
    // 把这次请求封装为一个函数：当年需要在调用时调用即可
    getData() {
      // 先测试接口返回的数据格式
    ...
    removeCategoryName() {
      ...
      // 到给服务器参数说明可有可无的；如果属性值为空的字符串还是要把相应的字段带给服务器
      // 但是把相应的字段改为undefined后，当前这个字段就不会带给服务器
      ...
      // 严谨：本意是删除query，如果路径出现params不应该删除，路径跳转的时应该带着
    ...
    // 排序的操作
    changeOrder(flag) {
      // flag下次：它是一个标记，代表用户点击的是综合（1）价格（2）【用户点击时传递进来的】
      // console.log(flag);
      let originOrder = this.searchParams.order; // 当前的状态
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 分类名字和关键字不用清空；因为每次路由发送变化时，都会赋予新的数据
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },

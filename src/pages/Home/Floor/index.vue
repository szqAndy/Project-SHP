...
export default {
  ...
  props: ["list"],
  mounted() {
    // 第一次书写Swiper时，在mounted当中书写是不可以的，但是为什么现在这里却可以？
    // 第一次写轮播图时，是在当前组件内部发请求，动态渲染解构【前台至少服务器数据需要回来】，因此之前的写法在这不行！
    // 现在的写法为什么可以？因为请求是父组件发动，父组件通过props传递过来的，而且结构都已经有了的情况下执行mounted
  },
